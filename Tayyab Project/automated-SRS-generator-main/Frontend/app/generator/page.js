...
import { Send, Menu, Plus, MessageSquare, FileText, Download, User, LogOut, PanelLeft } from "lucide-react";
...
function GeneratorPage() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Chat save is now handled by the backend in /api/chat
  ...
  // SRS Generate aur Download karne ka function
  const handleExportSRS = async () => {
    if (messages.length === 0) return alert("Please chat first to generate SRS.");
    ...
    try {
      ...
      if (data.success && data.srs) {
        ...
        const blob = new Blob([data.srs], { type: 'text/markdown' });
        const url = window.URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = 'Project_SRS.md';
        a.click();
      }
    } catch (error) {
  ...
  return (
    <div className="flex h-screen bg-gray-900 text-white overflow-hidden font-sans">
      ...
      <motion.aside 
        ...
        className="bg-gray-900 border-r border-white/10 flex flex-col overflow-hidden md:relative absolute z-50 h-full md:h-auto"
      >
        <div className="p-4 flex items-center gap-2">
          <button className="flex items-center gap-2 flex-1 px-4 py-3 bg-white/5 hover:bg-white/10 rounded-lg border border-white/10 transition text-sm">
            <Plus className="w-4 h-4" />
            <span>New Project</span>
          </button>
          <button 
            onClick={() => setSidebarOpen(false)}
            className="p-2 hover:bg-white/10 rounded-lg text-gray-400 shrink-0"
            title="Close Sidebar"
          >
      ...
      {/* Collapsed Sidebar Strip (Click to Open) */}
      {!isSidebarOpen && (
        <div 
          onClick={() => setSidebarOpen(true)}
          className="w-5 bg-gray-900 border-r border-white/10 hover:bg-white/5 cursor-pointer flex flex-col items-center justify-center transition-colors md:relative absolute h-full z-40 left-0"
          title="Open Sidebar"
        >
          <div className="h-10 w-1 bg-gray-600 rounded-full"></div>
        </div>
      )}
      ...
      <main className="flex-1 flex flex-col relative">
        ...
        <header className="flex items-center justify-between px-4 py-3 border-b border-white/10 bg-gray-900/50 backdrop-blur">
          ...
          <button 
            ...
            className={`p-2 hover:bg-white/10 rounded-lg text-gray-400 ${isGeneratingSRS ? 'animate-pulse text-blue-400' : ''}`} 
            title="Export SRS"
